...
var
  ...
  SongSelected: integer; //SongSelected retine pozitia melodiei selectate in playlist;
...
begin
 ...
 Main.FirstPlaylist.FileName.AddStrings(odAddSongsToPlaylist.Files); //Adauga denumirile fisierelor (+cailecatre ele) in structura
 Main.FirstPlaylist.ShownFileName.Clear; //Pentru a preveni retinerea aceluiasi fisier de 2+ ori, se sterge toata lista
 for I := 0 to Main.FirstPlaylist.FileName.Count-1 do
   Main.FirstPlaylist.ShownFileName.Add(ExtractFileName(Main.FirstPlaylist.FileName.Strings[i])); //Adauga numai denumirile fisierelor, fara cai, in structura
 lbPlaylist.Items.Clear; //Pentru a preveni afisarea de 2+ ori a aceluiasi fisier, se sterge lista
 lbPlaylist.Items.AddStrings(Main.FirstPlaylist.ShownFileName); //Adauga melodiile selectate in playlist
 SetLength(Main.FirstPlaylist.ID,Main.FirstPlaylist.FileName.Count); //Stabileste lungimea colectiei de ID-uri;
 for I := 0 to Length(Main.FirstPlaylist.ID)-1 do
   Main.FirstPlaylist.ID[i]:=Bass.BASS_StreamCreateFile(false,PChar(Main.FirstPlaylist.FileName[i]),0,0,BASS_UNICODE); //Incarca ID-urile
end;
...
begin
 i:=0;
 SongSelected:=-1; //SongSelected=-1 inseamna ca nicio melodie nu a fost aleasa
